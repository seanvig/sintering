...
predictions <- tidyr::gather(pred, key="model", value="y", 2:3)
...
plot<-ggplot() +
    geom_point(data = data, aes(x=t, y=y)) +
  stat_smooth(aes(x=data$t[c(1:i,i:length(data$t))], y=comb_pred), color="green", method="gam") +
  ...
  geom_point(aes(x=data$t[i:length(data$t)], y=predict(invlog_model)), color="red") 
plot
...
```{r}
ggplot(alumina, aes(x=t, y=y,  color=Dope, shape=Type)) +
  geom_point(size=2) +
